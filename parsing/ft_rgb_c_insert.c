...
/*   Updated: 2023/12/08 16:14:48 by sbellafr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_rgb_c_one(char *str, int i, t_data *data)
{
	...
	if ((data->ceiling.r > 255 || data->ceiling.r < 0) || ft_strlen(buffer) > 3)
	{
...
int	ft_rgb_c_two(char *str, int i, t_data *data)
{
	...
	if ((data->ceiling.g > 255 || data->ceiling.g < 0) || ft_strlen(buffer) > 3)
	{
		printf("error check the number in the rgb\n");
		exit(2);
	}
...
int	ft_rgb_c_three(char *str, int i, t_data *data)
{
	...
	if ((data->ceiling.b > 255 || data->ceiling.b < 0) || ft_strlen(buffer) > 3)
	{
		printf("error check the number in the rgb\n");
		exit(2);
	}
